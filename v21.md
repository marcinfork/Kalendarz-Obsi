...
// Domyślny folder to 'events', chyba że nie istnieje, wtedy pierwszy z listy
var state = {
...
// Funkcja pobierająca wszystkie foldery w Vault
function getAllFolders() {
    var folders = app.vault.getAllLoadedFiles()
        .filter(f => f.children) // Filtruje tylko foldery (pliki nie mają children)
        .map(f => f.path)
        .filter(p => p !== "/"); // Opcjonalnie usuń root, jeśli chcesz
    return folders.sort();
}

/* ---------- Data Loading (Dynamic) ---------- */
function reloadData() {
    // Zapytanie Dataview zależne od wybranego folderu
    ...
    state.events = dv.pages(source)
      ...
      .map(p => {
        ...
        if (!s) return null;
        return { file: p.file, title: p.file.name, start: s, end: e || s, icon: p.icon || "", color: p.color || "#b3e5fc" };
      })
      .filter(Boolean);
    ...
    state.routines = dv.pages(source)
      ...
      .map(p => {
        ...
        if (!d) return null;
        return { file: p.file, date: d, activities: p.activities || [], title: p.file.name };
      })
      .filter(Boolean);
}
...
// Aktualizacja daty zakończenia (RIGHT RESIZE)
...
async function createNewEventFile(newStartDate) {
    ...
    // Użyj wybranego folderu. Jeśli root, nie dodawaj slasha na początku
...
`;
    ...
    try {
        ...
        // Poczekaj chwilę na indeksowanie i odśwież
        setTimeout(() => { reloadData(); render(dv.container); }, 300);
    } catch (e) {
        console.error("Błąd:", e);
        alert(`Błąd: Upewnij się, że folder '${state.selectedFolder}' istnieje.`);
    }
}
...
function render(container) {
  ...
  createViewButton("month", "📅", "Widok Miesięczny");
  createViewButton("year-scrollable", "📜", "Widok Roczny (Przewijany)");
  createViewButton("year-grid", "📊", "Widok Roczny (Siatka)");
  
  // --- DROPDOWN WYBORU FOLDERU ---
  var folderSelect = actionControls.createEl("select", { cls: "folder-dropdown" });
  var allFolders = getAllFolders();
  
  // Jeśli folderu z ustawień nie ma na liście (np. usunięty), ustaw pierwszy dostępny
  ...
  folderSelect.onchange = async (e) => {
      ...
      // Self-modifying code: Zapisz wybór bezpośrednio w kodzie tego pliku
      try {
        var file = app.vault.getAbstractFileByPath(dv.current().file.path);
        var content = await app.vault.read(file);
        
        // Regex szukający linii: selectedFolder: "tekst",
        // Zmieniamy tylko pierwsze wystąpienie w obiekcie state
        var updatedContent = content.replace(/(selectedFolder:\s*")(.+?)(")/, `$1${newFolder}$3`);
        
        await app.vault.modify(file, updatedContent);
      } catch (err) {
        console.error("Nie udało się zmodyfikować kodu:", err);
      }
  ...
  var fullScreenBtn = actionControls.createEl("button", { 
      text: state.fullWidth ? "🤏" : "↔️", 
      cls: state.fullWidth ? "active-view" : "" 
  });
  fullScreenBtn.title = state.fullWidth ? "Przywróć szerokość" : "Pełna szerokość";
  ...
  // --- PRZYCISK PLUS ---
  var addBtn = actionControls.createEl("button", { text: "➕", cls: "add-event-btn" });
...
function renderMonth(container, navContainer) {
  var prevBtn  = navContainer.createEl("button", { text: "⬅" });
  var title    = navContainer.createEl("span",   { text: state.date.format("MMMM YYYY"), cls: "month-title" });
  var nextBtn  = navContainer.createEl("button", { text: "➡" });
  var todayBtn = navContainer.createEl("button", { text: "Dziś" });
  ...
  ["Pon","Wto","Śro","Czw","Pią","Sob","Nie"].forEach(d => 
...
function renderWeek(container, start, isGridMode = false) {
  ...
  weekEvents.forEach(ev => {
    ...
    if (!isGridMode) { 
        ...
        // Resize Handlers (uproszczone dla czytelności, logika taka sama jak w poprzednim kodzie)
        var createResizeHandler = (side) => {
             ...
             handle.addEventListener('mousedown', function(e) {
                ...
                function onMouseMove(moveE) {
                    // Wizualne przesuwanie (opcjonalne, w poprzednim kodzie było dokładne)
                }
...
function renderYearScrollable(container, navContainer) {
  navContainer.createEl("button", { text: "◀", onclick: () => { state.date = state.date.clone().subtract(1, "year"); render(dv.container); } });
  navContainer.createEl("span", { cls: "month-title", text: state.date.format("YYYY") });
  navContainer.createEl("button", { text: "▶", onclick: () => { state.date = state.date.clone().add(1, "year"); render(dv.container); } });
  navContainer.createEl("button", { text: "Dziś", onclick: () => { state.date = window.moment(); render(dv.container); } });
  ...
  for (var m = 0; m < 12; m++) {
    ...
    // Event rendering for scroll view (uproszczone: używa state.events)
...
function renderYearGrid(container, navContainer) {
  ...
  for (var m = 0; m < 12; m++) {
    ...
    ["Pon","Wto","Śro","Czw","Pią","Sob","Nie"].forEach(d => daysRow.createEl("div", { text: d, cls: "day-name-small" }));
...
.day-number { font-size:0.8em; font-weight:bold; }
